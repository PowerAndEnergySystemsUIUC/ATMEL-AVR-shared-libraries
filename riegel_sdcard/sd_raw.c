...
static unsigned long raw_block_address;
...
static int _sd_infodirty;
static struct sd_raw_info _sd_infocache;
...
#ifdef RIEGEL_SDCARD_VERBOSE_DEBUG
#include "../fleury_uart/fleury_uart.h"
#define SD_DEBUG(x) printf(x)
#else
#define SD_DEBUG(x)
#endif
 ...
 */
uint8_t sd_raw_read(unsigned long offset, uint8_t* buffer, uint16_t length)
{
 ...
 */
uint8_t sd_raw_read_interval(unsigned long offset, uint8_t* buffer, uint16_t interval, uint16_t length, sd_raw_read_interval_handler_t callback, void* p)
{
 ...
 */
uint8_t sd_raw_write(unsigned long offset, const uint8_t* buffer, uint16_t length)
{
    if(sd_raw_locked())
        return 0;
 ...
 */
uint8_t sd_raw_write_interval(unsigned long offset, uint8_t* buffer, uint16_t length, sd_raw_write_interval_handler_t callback, void* p)
{
#if SD_RAW_SAVE_RAM
    ...
    for(i = 0; i < 18; ++i)
    {
        ...
        else
        {
            ...
            if(sd_raw_card_type & (1 << SD_RAW_SPEC_2))
            {
                ...
                {
                    ++csd_c_size;
                    info->capacity = (unsigned long) csd_c_size * 512 * 1024;
                }
            }
            else
#endif
...
void sd_raw_set_cached_info_dirty(void)
{
   _sd_infodirty = 1;
}
struct sd_raw_info* sd_raw_get_cached_info(void)
{
   if(_sd_infodirty)
   {
      if(sd_raw_get_info(&_sd_infocache))
	     _sd_infodirty = 0;
	  else
	  	 return NULL;
   }
   return &_sd_infocache;
}
