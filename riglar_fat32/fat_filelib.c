...
static FL_FILE* _allocate_file(void)
{
	...
	if (file )
	{
        ...
        if(file != _open_file_list)
		{
			// Add to open list if it is not already there
...
//-----------------------------------------------------------------------------
// _create_file: Create a new file.  If it already exists, delete it, and create a new one.
//-----------------------------------------------------------------------------
#ifdef FATFS_INC_WRITE_SUPPORT
...
int fl_format(void)
{
   return fatfs_format_fat16(&_fs, media_capacity() / FAT_SECTOR_SIZE, "PESADE");
}
//-----------------------------------------------------------------------------
// fl_get_fs:
//-----------------------------------------------------------------------------
